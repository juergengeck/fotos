...
const START_TIMEOUT_MS = Number(process.env.FOTOS_SMOKE_SERVER_TIMEOUT_MS || 90_000);
...
async function main() {
  ...
  const storageDir = mkdtempSync(path.join(tmpdir(), 'fotos-browser-sync-headless-'));
  const browserBuildDir = mkdtempSync(path.join(tmpdir(), 'fotos-browser-sync-dist-'));

  const commServer = spawn(process.execPath, [COMM_SERVER_BUNDLE, '-h', 'localhost', '-p', String(commPort), '-l'], {
    cwd: REPO_ROOT,
  ...
  const headless = spawn(
    ...
    [
      ...
      '--storage', storageDir,
      '--comm-server', commUrl,
      '--email', 'fotos-browser-sync@test.local',
      '--password', 'fotos-browser-sync-pass',
      '--name', 'FotosBrowserSync',
      '--ephemeral',
    ],
    {
      cwd: HEADLESS_ROOT,
  ...
  try {
    ...
    previewServer.stdout?.on('data', prefixOutput(process.stdout, '[vite-preview] '));
    previewServer.stderr?.on('data', prefixOutput(process.stderr, '[vite-preview:err] '));

    await waitForHttp(viteUrl, START_TIMEOUT_MS);

    const integrationRun = spawn(process.execPath, [INTEGRATION_SCRIPT], {
      cwd: BROWSER_UI_ROOT,
      stdio: 'inherit',
      env: {
        ...process.env,
        FOTOS_LIVE_URL: viteUrl,
      },
    });

    const exitCode = await new Promise(resolvePromise => {
      integrationRun.once('exit', code => resolvePromise(code ?? 1));
    });
